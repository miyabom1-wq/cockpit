...
export async function getStockList(env,market){
  ...
  if(raw){const x=parseJson(raw,[]);if(Array.isArray(x))return x.map(clean=>cleanItem(clean,m)).filter(Boolean).map((it,i)=>({...it,focus_tier:focusTier(m,i)}));}
...
export async function handleStockListAction(env,market,body={}){
  ...
  if(action==='add'){
    ...
    list.push({symbol,name,added_at:nowIso(),source:'manual',pinned:body.pinned!==false});await saveStockList(env,m,list);return{ok:true,added:true,item:{...list.at(-1),focus_tier:focusTier(m,list.length-1)},count:list.length};
  }
  if(action==='batch_add'){
    const added=[],skipped=[];for(const it of Array.isArray(body.items)?body.items:[]){const symbol=normalizeSymbol(it.symbol,m);if(!symbol||list.some(x=>x.symbol===symbol)){skipped.push({symbol,reason:'登録済みまたは形式不正'});continue;}if(list.length>=maxFor(m)){skipped.push({symbol,reason:'上限到達'});continue;}const item={symbol,name:String(it.name||symbol).slice(0,80),added_at:nowIso(),source:String(it.source||'manual'),pinned:it.pinned!==false};list.push(item);added.push(item);}
    if(added.length)await saveStockList(env,m,list);return{ok:true,added,skipped,count:list.length};
  }
  if(action==='delete'){
  ...
  if(action==='promote'){
    const symbol=normalizeSymbol(body.symbol,m),idx=list.findIndex(x=>x.symbol===symbol);if(idx<0)return{ok:false,error:'not found'};const boundary=m==='jp'?LIMITS.jpCore:LIMITS.usLead;const [it]=list.splice(idx,1);list.splice(Math.min(boundary-1,list.length),0,it);await saveStockList(env,m,list);return{ok:true,count:list.length};
  }
  if(action==='demote'){
    const symbol=normalizeSymbol(body.symbol,m),idx=list.findIndex(x=>x.symbol===symbol);if(idx<0)return{ok:false,error:'not found'};const boundary=m==='jp'?LIMITS.jpCore:LIMITS.usLead;const [it]=list.splice(idx,1);list.splice(Math.min(boundary,list.length),0,it);await saveStockList(env,m,list);return{ok:true,count:list.length};
  }
  if(action==='replace'){
    const next=[];for(const it of Array.isArray(body.items)?body.items:[]){const x=cleanItem(it,m);if(x&&!next.some(y=>y.symbol===x.symbol)&&next.length<maxFor(m))next.push(x);}
    await saveStockList(env,m,next);return{ok:true,count:next.length};
  }
