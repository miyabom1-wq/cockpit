...
function normalizeSymbols(value){const xs=Array.isArray(value)?value:String(value||'').split(/[\s,、/]+/);return[...new Set(xs.map(x=>{const s=String(x||'').trim().toUpperCase();if(!s)return null;return normalizeSymbol(s,/\.T$/.test(s)||/^[0-9A-Z]{4}$/.test(s)?'jp':'us');}).filter(Boolean))].slice(0,20);}
function normalizeEvent(x={}){return{...x,symbols:normalizeSymbols(x.symbols||[]),source:x.source||'manual'};}
export async function getEvents(env){const v=parseJson(await env.COCKPIT_KV.get(KEYS.events),[]);return(Array.isArray(v)?v:[]).map(normalizeEvent);}
...
export async function mutateEvent(env,body={}){
  ...
  if(action==='add'){
    ...
    const item={id:`e${Date.now()}${Math.random().toString(36).slice(2,6)}`,name,time,category:String(body.category||'other').slice(0,20),symbols:normalizeSymbols(body.symbols),source:'manual',pinned:false,created_at:nowIso()};list.push(item);list.sort((a,b)=>new Date(a.time)-new Date(b.time));await save(env,list);return{ok:true,event:item};
  }
