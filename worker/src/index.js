...
import { scheduleSnapshotOptions, marketParts, runStageBatch, getStage } from './services/stage.js';
...
async function scheduledStage(env){const{minute,nodes}=scheduleNodes();for(const n of nodes){if(minute<n.at||minute>=n.at+55)continue;const marker=`sched:v35:${n.key}:${n.tradeDate}`;if(await env.COCKPIT_KV.get(marker))continue;
    try{
      if(n.action==='explorer')await buildExplorer(env,'jp',true);
        ...
        await getEnrichedRanking(env,n.market,true);if(n.kind==='confirmed')await captureSignalLog(env,n.market,'auto');
      }else{
