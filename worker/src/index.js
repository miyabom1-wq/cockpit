...
export function scheduleNodes(now=new Date()){
  ...
  const add=(market,label,at,kind,tradeDate,parts,action='stage',guard={})=>{if(action==='stage'){for(let p=1;p<=parts;p++)nodes.push({key:`${label}:b${p}`,at,market,kind,tradeDate,parts,part:p,...guard});nodes.push({key:`${label}:enrich`,at,market,kind,tradeDate,parts,action:'enrich'});}else nodes.push({key:label,at,market,kind,tradeDate,parts,action});};
