...
test('three tabs keep distinct roles and Today shows schedule plus compact market snapshot',()=>{
  ...
  assert.match(html,/\['GOLD','金'\]/);
  assert.match(html,/\['BTC','BTC'\]/);
  assert.match(html,/\['日経','日経平均'\],[\s\S]*?\['日経先物','日経先物（CME円建て）'\]/);
  assert.match(html,/\['S&P500','S&P500'\],[\s\S]*?\['NASDAQ','Nasdaq'\]/);
  assert.match(html,/\['KOSPI','韓国KOSPI'\],[\s\S]*?\['KOSDAQ','韓国KOSDAQ'\]/);
  assert.match(html,/\['SOX','SOX'\],[\s\S]*?\['VIX','VIX'\]/);
  assert.match(html,/\['USD\/JPY','ドル円'\],[\s\S]*?\['米10年','米10年債'\]/);
  assert.match(html,/\['GOLD','金'\],[\s\S]*?\['BTC','BTC'\]/);
